...
listeners:
    ...
    - name: mcp-servers # no host name required as the host is set by the HTTRoute
      port: 8080
      protocol: HTTP
      hostname: '*.mcp.local' # this hostname can be any wildcard hostname. It is here to ensure the MCP Server routes attach only to this listener and can be protected by the correct AuthPolicy. Alternatively, the MCP Server routes can specify parentRefs.sectionName matching this listener.
      allowedRoutes:
        namespaces:
...
- **Individual MCP Server Routes:** These are intended to route to individual MCP Servers that can handle distinct tools/calls from a client. There can be many of these routes but there is expected to be a 1:1 relationship between a route and a MCP Backend. Each MCP Server route should have some form of hostname set and it should match the gateway listener (in the above example `*.mcp.local`). The hostname used, is not hugely important as it is not expected to be DNS resolvable. In our examples we use `{ServerName}.mcp.local` in the HTTPRoute. Each route should have a single rule that points at the MCP backend. [Example](../../config/test-servers/server1-httproute.yaml). Alternatively, no hostname has to be specified at the listener at all, in which case the all HTTPRoute objects must specify `parentRefs.sectionName` to ensure attachment only to the intended listener.
